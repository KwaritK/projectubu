...
const roomTimers = {};
const rooms = new Map();
const activeConnections = new Map(); // เพิ่มการเก็บ active connections
...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    .find(s => s.username === username && s.roomID === roomID && s.id !== socket.id);

  if (existingSocket) {
    // ถ้ามีการเชื่อมต่อเดิม ให้ตัดการเชื่อมต่อเก่า
    existingSocket.disconnect();
      ...
      const existingConnection = activeConnections.get(`${username}-${roomID}`);
      if (existingConnection) {
        console.log(`Disconnecting existing connection for ${username} in room ${roomID}`);
        const existingSocket = io.sockets.sockets.get(existingConnection);
        if (existingSocket) {
          existingSocket.disconnect();
        }
        activeConnections.delete(`${username}-${roomID}`);
      }

      // บันทึกการเชื่อมต่อใหม่
      activeConnections.set(`${username}-${roomID}`, socket.id);

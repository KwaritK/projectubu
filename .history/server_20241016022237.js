...
const roomTimers = {};
const rooms = new Map(); // เปลี่ยนจาก users เป็น rooms เพื่อเก็บข้อมูลทั้งหมดของห้อง
...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    แonst unbanExpiredUsers = async () => {
      try {
        const now = new Date();
        await User.updateMany(
          { isBanned: true, banEnd: { $lte: now } },  // ค้นหาผู้ใช้ที่ถูกแบนและมีเวลาหมดอายุ
          { 
            isBanned: false, 
            $unset: { banEnd: "" }  // ลบฟิลด์ banEnd เมื่อตรวจสอบการยกเลิกแบนเสร็จแล้ว
          }
        );
        console.log("Unbanned users with expired bans.");
      } catch (error) {
        console.error("Error unbanning expired users:", error);
      }
    };
    // ตั้งเวลาให้รันฟังก์ชันทุกๆ ชั่วโมง (ใช้ '* * * *' สำหรับทดสอบทุกนาที)
    cron.schedule(' * * * *', async () => {  // รูปแบบนี้จะรันทุกๆ ชั่วโมง

...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('createRoom', async ({ username, roomID }, callback) => {
      try {
        const newRoom = new Room(roomData); // สร้างห้องใหม่
        await newRoom.save(); // บันทึกห้องใหม่ในฐานข้อมูล

        // ส่ง roomID กลับไปยัง client
        socket.emit('roomCreated', { roomID: newRoom.roomID, success: true });
    } catch (error) {

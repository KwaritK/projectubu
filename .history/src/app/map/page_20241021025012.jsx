...
const MapPage = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // ตรวจสอบสถานะแบนทุกๆ 5 วินาที
      if (status === 'authenticated' && session?.user?.isBanned) {
        router.replace('/banned'); // redirect ไปหน้า banned ถ้าผู้ใช้ถูกแบน
        clearInterval(interval);  // หยุด interval หลังจาก redirect แล้ว
      }
  ...
  if (status === 'loading') {
    return <p>Loading...</p>;  // แสดงหน้า loading ขณะตรวจสอบ session
  }

...
const ChatRoom = () => {
  ...
  useEffect(() => {
    ...
    socket.on('userJoined', (username) => {
      setMessages((prevMessages) => [...prevMessages, { type: 'system', text: `${username} เข้าร่วมแชท`, time: new Date() }]);
    });

    socket.on('userLeft', (username) => {
      setMessages((prevMessages) => [...prevMessages, { type: 'system', text: `${username} ออกจากแชท`, time: new Date() }]);
    });

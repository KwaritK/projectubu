              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all relative">
                ...
                <div className="flex justify-between mb-4">
                  <button
                    ...
                    className={'w-[48%] px-2 py-2 border rounded-md hover:bg-gray-100 flex items-center justify-center ${roomType === one-on-one' ? 'bg-blue-500 text-white' : 'hover:bg-gray-100'}`}
                  >
                    <Image src={one} alt="คุยแบบตัวต่อตัว" width={150} height={50} />
                  </button>
                  <button
                    onClick={() => setRoomType('multi')}
                    className={"w-[48%] px-2 py-2 border rounded-md hover:bg-gray-100 flex items-center justify-center ${roomType === multi' ? 'bg-blue-500 text-white' : 'hover:bg-gray-100'}`}
                  >

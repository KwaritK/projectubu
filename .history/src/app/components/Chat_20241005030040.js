  ...
  useEffect(() => {
    ...
    socket.on('connect', () => { //ตรวจสอบการกำหนดในระบบ
      ...
      socket.on('roomCreated', (data) => {
        if (data.success) {
            console.log(`ห้องสร้างสำเร็จด้วย roomID: ${data.roomID}`);
            router.push(`/roomtag/${data.roomID}`); // เปลี่ยนเส้นทางไปยังห้องที่สร้างขึ้น
        } else {
            console.log('เกิดข้อผิดพลาดในการสร้างห้อง');
        }
    });

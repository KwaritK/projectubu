...
const CreateRoomModal = ({ isOpen, closeModal,username }) => { //รับชื่อผุ้ใช้
  ...
  const handleCreateRoom = async (type) => {
    ...
    try {
      const res = await fetch('/api/createroom', {
        ...
        body: JSON.stringify({ roomType, ageGroup, username }), // ส่ง roomType และ ageGroup
      });
  ...
  return (
    <Transition appear show={isOpen} as={Fragment}>
      ...
      <h2 className="text-lg font-medium mb-4">สวัสดี {username}! เลือกรูปแบบห้องที่คุณต้องการสร้าง</h2>
        <Transition.Child
          as={Fragment}

...
const CreateRoomModal = ({ isOpen, closeModal }) => {
  ...
  return (
    <Transition appear show={isOpen} as={Fragment}>
      ...
      >
        <Image
          src="/path/to/one-on-one-image.jpg"
          alt="คุยแบบตัวต่อตัว"
          layout="fill"
          objectFit="cover"
          className={imageClass('one-on-one')}
        />
        <span className={`relative z-10 ${roomType === 'one-on-one' ? 'font-bold' : ''}`}>One-on-One</span>
      </button>
      ...
      >
        <Image
          src="/path/to/multi-image.jpg"
          alt="คุยแบบกลุ่ม"
          layout="fill"
          objectFit="cover"
          className={imageClass('multi')}
        />
        <span className={`relative z-10 ${roomType === 'multi' ? 'font-bold' : ''}`}>Multi</span>
      </button>
    </div>
  );
};

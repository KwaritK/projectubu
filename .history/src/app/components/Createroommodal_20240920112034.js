...
const CreateRoomModal = ({ isOpen, closeModal }) => {
  ...
  <button
    ...
    className={`w-[48%] px-4 py-2 rounded-md flex flex-col items-center justify-center transition-all duration-300 ease-in-out';
       ${roomType === 'one-on-one' ? 'text-blue-500' : 'text-gray-400'}`}
       style={{ height: roomType === 'one-on-one' ? 'scale-110' : 'scale-100 transition-all duration-300 ease-in-out'}}
  >

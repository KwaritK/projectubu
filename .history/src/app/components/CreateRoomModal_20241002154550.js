...
import SetUsername from './SetUsername'; // นำเข้า SetUsername

const CreateRoomModal = ({ isOpen, closeModal, onSetUsername  }) => {
  const router = useRouter();
  const [error, setError] = useState('');
  const [ageGroup, setAgeGroup] = useState('');
  const [roomType, setRoomType] = useState('');
  const [username, setUsername] = useState(''); // เพิ่ม state สำหรับเก็บค่าชื่อผู้ใช้

...
const CreateRoomModal = ({ isOpen, closeModal }) => {
  ...
  <button
    ...
    className={`w-full px-4 py-2 border rounded-md hover:bg-gray-100 flex items-center justify-center relative w-[48%] rounded-md overflow-hidden transition-all duration-300 ${
      roomType === 'one-on-one' ? 'bg-blue-500' : 'bg-gray-200 scale-90'
    }`}
    style={{ height: roomType === 'one-on-one' ? '150px' : '100px' }}
  >
    <Image
      src={one}
      alt="คุยแบบตัวต่อตัว"
  ...
  <button
    ...
    className={`relative w-[48%] rounded-md overflow-hidden transition-all duration-300 ${
      roomType === 'multi' ? 'bg-blue-500' : 'bg-gray-200 scale-90'
    }`}
    style={{ height: roomType === 'multi' ? '150px' : '100px' }}
  >
    <Image
      src={multi}
      alt="คุยแบบกลุ่ม"

...
const ChatBoard = ({ username, initialRoomID }) => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => { //ตรวจสอบการกำหนดในระบบ  
      ...
      if (roomID) {
        const roomID = date.getMinutes();
        socket.emit('joinRoom', { username, roomID }, (response) => {
          if (response.success) {
            console.log('เข้าร่วมห้องสำเร็จ');
          } else {
            console.log('เข้าร่วมห้องไม่สำเร็จ:', response.message);
          }
        });
      }
    });
  ...
  const sendMessage = (e) => {
    ...
    if (message.trim()) {
      const formattedTime = formatTime(new Date());
      socket.emit('message', { text: message, roomID, sender: username, time: formattedTime });
      setMessage('');
    }
  };

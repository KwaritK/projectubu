...
const CreateRoomModal = ({ isOpen, closeModal }) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // เรียก API checkuser เพื่อเช็คจำนวนผู้ใช้ในห้อง
      const checkUserRes = await fetch('/api/checkuser', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ roomCode: roomID, userID: 'user123' }), // ส่ง roomCode และ userID
      });

      const checkUserData = await checkUserRes.json();

      if (checkUserRes.ok && checkUserData.success) {
        // ถ้า API ตรวจสอบสำเร็จและห้องมีที่ว่าง
        router.push(`/roomtag/${roomID}`);
      } else {
        setError(checkUserData.error || 'ไม่สามารถเข้าร่วมห้องได้');
      }
    } else {

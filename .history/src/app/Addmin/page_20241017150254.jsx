...
function AdminReportsPage() {
  ...
  const handleBanSuccess = useCallback(() => {
    fetchReports();// ตรวจสอบว่า fetchReports อัปเดตข้อมูลสถานะใหม่
  }, [fetchReports]);

  const getStatusColor = (status) => {
    switch(status) {
      case 'pending': return 'text-yellow-500';
      case 'banned': return 'text-green-500';
              ...
              <tbody>
                  {reports.map((report) => (
                    <tr key={report._id} className="hover:bg-gray-50">
                      <td className="py-3 px-4 border-b text-center">{report.reportedUser?.email || 'Unknown'}</td>
                      <td className="py-3 px-4 border-b text-center">{report.reportedBy?.email || 'Unknown'}</td>

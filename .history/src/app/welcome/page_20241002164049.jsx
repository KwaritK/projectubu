...
import SetUsername from '../components/SetUser'; // นำเข้าตัวกำหนดชื่อผู้ใช้
...
const WelcomePage = () => {
  ...
  // ดึงชื่อผู้ใช้จาก localStorage เมื่อโหลดหน้า
  useEffect(() => {
  ...
  const handleSetUsername = (name) => {
    setUsername(name);
    localStorage.setItem('username', name); // เก็บชื่อใน localStorage
  };
  ...
  // ตรวจสอบว่ามี username แล้วหรือยัง
  if (!username) {
    return <SetUsername onSetUsername={handleSetUsername} />;
  }

  const handleJoinRoom = async (e) => { // ฟังก์ชันของปุ่มในการกรอกโค้ดเพื่อเข้าร่วมห้อง
    e.preventDefault();

...
const WelcomePage = () => {
  ...
  const handleJoinRoom = async (e) => {
    ...
    try {
      ...
      if (result.exists) {
        // ตรวจสอบเงื่อนไขเพิ่มเติม (ประเภทห้อง, จำนวนผู้ใช้, ช่วงอายุ)
        const canJoin = await checkRoomConditions(roomCode);
        if (canJoin) {
          router.push(`/roomtag/${roomCode}`);
        } else {
          setErrorMessage("You cannot join this room due to restrictions.");
        }
      } else {
  ...
  const checkRoomConditions = async (roomCode) => {
    // ส่ง request ไปยัง API เพื่อตรวจสอบเงื่อนไขของห้อง
    const response = await fetch(`/api/checkroomconditions?roomCode=${roomCode}`);
    const result = await response.json();

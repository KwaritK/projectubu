export default class Game extends Phaser.Scene {
  ...
  preload() {
      ...
      this.load.image('ob1', '/asset/imageR/ob1.png'); //งูพื้น
      this.load.image('ob2', '/asset/imageR/obstacle.png'); //หญ้า
      this.load.image('ob3', '/asset/imageR/ob2.png'); //งูบิน
      this.load.image('heart', '/asset/imageR/heart.png');
      this.load.atlas('run','/asset/imageR/run22.png','/asset/imageR/run22.json');
      this.load.atlas('dodgh','/asset/imageR/do.png','/asset/imageR/do.json');
      this.load.atlas('ene', '/asset/imageR/00.png', '/asset/imageR/00.json');
      this.load.atlas('obst','/asset/imageR/obst.png','/asset/imageR/obst.json');
      this.load.atlas('snakefly','/asset/imageR/snakefly.png','/asset/imageR/snakefly.json');
      this.load.atlas('snakeground','/asset/imageR/snakeground.png','/asset/imageR/snakeground.json');
  ...
  this.scoreText.setDepth(10);
      ...
      this.anims.create({
        ...
        frames: [
          ...
          //{ key: 'run', frame: 'run1.1.png' },
          //{ key: 'run', frame: 'run1.2.png' },
          //{ key: 'run', frame: 'run1.3.png' },
          { key: 'run', frame: 'run0.4.png' },          
          //{ key: 'run', frame: 'run0041.png' },
         // { key: 'run', frame: 'run0.4.png' },
          //{ key: 'run', frame: 'run0.2.png' },
          //{ key: 'run', frame: 'run005.png' },
          //{ key: 'run', frame: 'run004.png' },
          //{ key: 'run', frame: 'run0.0.png' },
          //{ key: 'run', frame: 'run0.01.png' },
          //{ key: 'run', frame: 'run0.02.png' },
          //{ key: 'run', frame: 'run0.03.png' },
          //{ key: 'run', frame: 'run0.04.png' },
          //{ key: 'run', frame: 'run0.05.png' },
        ],
        frameRate: 9,
        repeat: -1
      });
      ...
      this.anims.create({
        key: 'dodgh',
        frames: [
          { key: 'dodgh', frame: 'dodgh1.1.png' },
          { key: 'dodgh', frame: 'dodgh2.1.png' }
        ],
        frameRate: 8,
        repeat: -1
      });
      ...
      this.anims.create({
        key: 'ene',
        frames: [
          { key: 'ene', frame: "enemy3.png" },
          { key: 'ene', frame: "enemy4.png" },
          { key: 'ene', frame: "enemy5.png" }
        ],
        frameRate: 5,
        repeat: -1
      });
      ...
      this.anims.create({
        key: 'obst',
        frames: [
          { key: 'obst', frame: 'obstacle1.png' },
          { key: 'obst', frame: 'obstacle2.png' },
          { key: 'obst', frame: 'obstacle.png' }
        ],
        frameRate: 5,
        repeat: -1
      });
      ...
      this.anims.create({
        key: 'snakefly',
        frames: [
          { key: 'snakefly', frame: 'ob4.png' },
          { key: 'snakefly', frame: 'ob3.png' },
          { key: 'snakefly', frame: 'ob2.png' }
        ],
        frameRate: 5,
        repeat: -1
      });
      ...
      this.anims.create({
        key: 'snakeground',
        frames: [
          { key: 'snakeground', frame: 'ob11.png' },
          { key: 'snakeground', frame: 'ob12.png' },
          { key: 'snakeground', frame: 'ob1.png' }
        ],
        frameRate: 5,
        repeat: -1
      });
  ...
  this.physics.add.existing(this.ground, true);
      ...
      this.enemy.setScale(0.75);
      //this.enemy.setSize(this.enemy.width * 0.2, this.enemy.height * 0.4);
      

      // กำหนดขนาดฮิตบ็อกซ์แบบคงที่
    const hitboxWidth = this.enemy.width * 0.2;
    const hitboxHeight = this.enemy.height * 0.4;
    this.enemy.setSize(hitboxWidth, hitboxHeight);
    
    // ปรับตำแหน่งของฮิตบ็อกซ์ให้อยู่ตรงกลางของ sprite
    this.enemy.setOffset(
      (this.enemy.width - hitboxWidth) / 2,
      (this.enemy.height - hitboxHeight) / 2
    );

      this.enemy.x = this.player.x - 2000;
  ...
  update(time,delta) {
    ...
    //console.log('Current Score:', currentScore);
      ...
      this.back.tilePositionX += 10; // Adjust the scrolling speed as needed
...
duck() {
        ...
        //this.originalHitboxHeight = this.player.body.height;
          ...
          const hitboxWidth = this.player.displayWidth * 0.3; // อาจปรับความกว้างเล็กน้อย

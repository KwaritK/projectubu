...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {  // เพิ่มการประกาศ socket
    ...
    socket.on('joinRoom', async ({ username, email, roomID }, callback) => {
     ...
     // ตั้งค่างาน cron ให้รันทุกชั่วโมงเพื่อตรวจสอบการยกเลิกแบน
cron.schedule('0 * * * *', async () => { // รันทุกชั่วโมง
  try {
      await User.updateMany(
          { isBanned: true, banEnd: { $lte: new Date() } },
          {
              isBanned: false,
              banEnd: null,
              banReason: ''
      ...
      console.log('อัปเดตสถานะการแบนเรียบร้อย');
  } catch (error) {
      console.error('เกิดข้อผิดพลาดในการอัปเดตสถานะการแบน:', error);
  }
}); 
    ...
    socket.on('disconnect', async () => {
      if (socket.username && socket.roomID) {
        ...
        if (userCount === 0 && !roomTimers[socket.roomID] ) {
          // ตั้งค่าตัวจับเวลา 5 นาที (300,000 มิลลิวินาที) หากไม่มีผู้ใช้อยู่ในห้อง
          ...
          }, 50000); // ตั้งเวลาลบห้องเมื่อไม่มีผู้ใช้ใน 5 นาที 300000
        }
      }
    });
  });

...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    // ฟังก์ชันสำหรับผู้ใช้เข้าร่วมห้องส่วนตัว
    socket.on('joinUserRoom', (userId) => {
      socket.join(userId);
      console.log(`User ${userId} joined their personal room`);
    });

    // ฟังก์ชันสำหรับผู้ใช้เข้าร่วมห้องสนทนา
    socket.on('joinRoom', async ({ username, email, roomID }) => {
      ...
      // ตรวจสอบสถานะการแบนของผู้ใช้
      try {
        const user = await User.findOne({ email });
        if (user && user.isBanned) {
          console.log(`User ${email} is banned.`);
          socket.emit('joinRoomResponse', { success: false, message: 'You are banned from joining rooms' });
          return;
        }
      } catch (error) {
        console.error('Error checking ban status:', error);
        socket.emit('joinRoomResponse', { success: false, message: 'Error checking ban status' });
        return;
      }

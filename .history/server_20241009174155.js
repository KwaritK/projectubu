...
app.prepare().then(() => {
  ...
  server.post('/api/report', async (req, res) => {
    try {
      const response = await fetch('http://localhost:3000/api/report', {
        ...
        body: JSON.stringify(req.body) // ส่งข้อมูลจาก req.body ไปยัง API
      });
  
      const data = await response.json();
      res.status(response.status).json(data); // ส่งข้อมูลที่ได้กลับไปยัง client
    } catch (error) {
      console.error('Error calling /api/report:', error);
      res.status(500).send('Failed to call report API');
    }
  });
  ...
  io.on('connection', (socket) => {  // เพิ่มการประกาศ socket
    ...
    socket.on('setUsername', async ({ username, roomID,email }, callback) => {
      console.log('Username:', username,'Username:', username, 'RoomID:', roomID);  // เพิ่มเพื่อ debug
